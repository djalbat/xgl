...
The `coordinates` and `indexes` arguments are the important ones. Jiggle works with facets underneath the hood, which are triangles with a colour or texture together with a normal. The normal is used for shading. Facets are defined by passing tripes of indexes that refer to the coordinates given. In this case there are four coordinates, one for each corner of the square. Two facts need to be created to cover the square. Note that the first and third coordinates are re-used. It is essential that you get the coordinates and indexes right for any canvas element. This information is used to populate the WebGL rendering buffers and if it is wrong, weird WebGL errors will likely surface.
